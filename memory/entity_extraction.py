...
class EntityExtractionMemory(BasePostgresMemory):
    """实体提取与知识图谱构建器
    ...
    def _on_memory_sleep(self, event: Event) -> None:
        """处理记忆休眠事件 - 触发知识图谱整理"""
    ...
    def consolidate_all_memories(self) -> dict:
        ...
        try:
            ...
            for i in range(0, len(memories), BATCH_SIZE):
                ...
                logger.info(f"批次 {batch_count} 完成: {result['nodes']} 节点, {result['edges']} 边")

            logger.info(f"知识图谱整理完成: {batch_count} 批次, {total_nodes} 节点, {total_edges} 边")
                ...
                return [
                    {
                        ...
                        "insight": row[2],
                        "importance": row[3],
                        "time": row[4].isoformat() if hasattr(row[4], "isoformat") else str(row[4]),
                        "metadata": row[5],
                    }
                    for row in rows
                ]
    ...
    def _build_summaries(self, memories: list[dict]) -> str:
        """将记忆列表构建为摘要文本"""
        summaries = []
        ...
        try:
            ...
            for item in extraction_result:
                ...
                if operation == "upsert_node":
                    if self._process_node(item):
                        node_count += 1
                elif operation == "upsert_edge":
                    if self._process_edge(item):
                        edge_count += 1
    ...
    def _mark_memories_consolidated(self, memory_ids: list[int]) -> None:
        ...
        query = """
            ...
            WHERE id = ANY(%s)
        """
        if self._execute_write(query, (memory_ids,)):
